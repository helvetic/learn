...
new Promise(function(resolve) {
  ...
  results.addEventListener('click', function(e) {
    ...
    if (e.target.getAttribute('data-role') === 'progressbox') {
      /*
        При нажатии на прогрессбар воспроизводится выбранная песня на выбранном времени
        (реагирует в том числе и на нажатие на прогрессбар другой песни)
       */
      var currentItem = e.target.parentNode.parentNode.parentNode.parentNode;
      let progressBox = e.target;
      let duration = globalPlayer.duration;
      let song = currentItem.querySelector('[data-role=playback]').dataset.src;

      onPause();
      globalPlayer.src = song;
      globalPlayer.play();
      globalPlayer.currentTime = duration * e.offsetX/progressBox.clientWidth;
      playingItem = currentItem;
    }
